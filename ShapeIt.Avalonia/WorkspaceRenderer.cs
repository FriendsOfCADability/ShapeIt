...
{
    /// <summary>
    /// Avalonia port of the WinForms WorkspaceRenderer (ShapeIt/WorkspaceRenderer.cs, excluded from this
    /// head): renders a collection of GeoObjects to a PNG bitmap with a <see cref="PaintToOpenGL"/>.
    ///
    ...
    /// <see cref="OffscreenPainter"/>'s business. Unlike the WinForms version, which can create a hidden
    /// window with a context of its own, this one always needs a visible OpenGL view: Avalonia owns the
    /// context and hands it out only inside its render pass.
    /// </summary>
    internal static class WorkspaceRenderer
    {
        ...
        /// interface there is no frame, and then there is no image either.
        /// </param>
            ...
            out string? unavailableReason)
        {
            ...
            {
                // Avalonia's Bitmap.Save writes PNG; there is no format argument.
                bitmap.Save(ms);
        ...
        private static WriteableBitmap? RenderOnPainterThread(
            PaintToOpenGL painter,
            IEnumerable<(IGeoObject obj, CADability.Substitutes.Color color)> coloredObjects,
            GeoVector viewDirection,
            int width,
            int height)
