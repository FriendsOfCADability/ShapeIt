...
namespace ShapeIt
{
    internal class SolidPlaneIntersectionPathAction : ConstructAction
    {
        ...
        public override void OnDone()
        {
            if (base.ActiveObject != null)
            {
                using (Frame.Project.Undo.UndoFrame)
                {
                    ...
                    List<ICurve> lcrvs = new List<ICurve>(crvs);
                    RemoveDuplicates(lcrvs, solidToIntersect.GetExtent(0.0));
                    if (crvs != null)
                    {
                    ...
                    List<Path> paths = Path.FromSegments(lcrvs);
        ...
        private void RemoveDuplicates(List<ICurve> lcrvs, BoundingBox extent)
        {
            ...
            for (int i = 0; i < lcrvs.Count; i++)
            {
                ot.AddObject(lcrvs[i] as IOctTreeInsertable);
            }
            // maybe we should use an octtree, but here the quick implementation
            for (int i = lcrvs.Count - 1; i > 0; --i)
            {
                ot.RemoveObject(lcrvs[i] as IOctTreeInsertable);
                IOctTreeInsertable[] found = ot.GetObjectsCloseTo(lcrvs[i] as IOctTreeInsertable);
                for (int j = 0; j < found.Length; ++j)
                {
                    if (lcrvs[i].SameGeometry(found[j] as ICurve, Precision.eps))
                    {
                        lcrvs.RemoveAt(i);
                        break;
                    }
