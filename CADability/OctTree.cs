...
namespace CADability
{
    ...
    public class OctTree<T> where T : IOctTreeInsertable
    {
        ...
        public class Node<TT> : IComparable<Node<TT>> where TT : T
        {
            ...
            internal void GetObjectsFromRect(Projection projection, BoundingRect rect, bool onlyInside, HashSet<TT> addToList)
            {
            ...
            internal void GetObjectsFromRect(Projection.PickArea area, bool onlyInside, HashSet<TT> addToList)
            {
                if (area == null) return;
...
#region IComparable<Node<TT>> Members
        ...
        public GeoObjectList DebugCubesByObject(HashSet<T> objs)
        {
            ...
            foreach (Node<T> node in Leaves)
            {
                if (node.list != null)
                {
                    if (objs.Intersection(new HashSet<T>(node.list)).Count > 0)
                    {
                        res.Add(node.cube.AsBox);
                    }
                }
