    ...
    */
        ...
        , IPropertyEntry
    {
        ...
        public class GeoPointInput : InputObject, IInputObject
        {
            ...
            /// <summary>
            /// When true and a point on the surface of a face is used as snapPoint, this surface
            /// (at the mouse point) defines the drawing plane for this action
            /// </summary>
            public bool canOverrideDrawingPlane;
            /// <summary>
            ...
            {   // die Maus hat sich bewegt
                ...
                SnapPointFinder.DidSnapModes DidSnap;
                GeoPoint p = constructAction.SnapPoint(e, vw, out DidSnap);
                if (canOverrideDrawingPlane && DidSnap == SnapPointFinder.DidSnapModes.DidSnapToFaceSurface && vw.LastSnapPlane.IsValid()) constructAction.OverrideDrawingPlane = vw.LastSnapPlane;
                // for inputs with drawing plane override capability, set the drawing plane according to the snapped face
                // reset it only if the snap was not to a face surface and canOverrideDrawingPlane is true.
                // for inputs with canOverrideDrawingPlane==false, the drawing plane is not changed.
                if (canOverrideDrawingPlane && DidSnap != SnapPointFinder.DidSnapModes.DidSnapToFaceSurface) constructAction.OverrideDrawingPlane = Plane.Invalid;
        ...
        public virtual void OnDone()
        {
            ...
            foreach (IView vw in Frame.AllViews)
            {
                if (vw is IActionInputView)
                {
                    (vw as IActionInputView).SetAdditionalExtent(BoundingBox.EmptyBoundingBox);
                }
            }
