...
namespace CADability.UserInterface
{
    ...
    public class ShowPropertyPolyline : PropertyEntryImpl, IDisplayHotSpots, ICommandHandler, IGeoObjectShowProperty
    {
        ...
        private GeoPointProperty polygonCenterProperty; // wg. hotspot merken 
        ...
        private GeoPointHotSpot polygonOuterRadiusHotSpot;
        private GeoPointHotSpot polygonInnerRadiusHotSpot;
        ...
        public ShowPropertyPolyline(Polyline polyline, IFrame frame) : base(frame)
        {
            ...
            if (polyline.IsRegularPolygon) base.resourceIdInternal = "Polyline.Polygon";
            InitSubEntries();
        }
        private void InitSubEntries()
        {
            ...
            else if (polyline.IsRegularPolygon)
            {
                subEntries = new IPropertyEntry[2];
                polygonCenterProperty = new GeoPointProperty(Frame, "Polygon.Center");
                polygonCenterProperty.OnGetValue = () => polyline.PolygonPlane.Location;
                polygonCenterProperty.OnSetValue = p => polyline.PolygonPlane = new Plane(p, polyline.PolygonPlane.DirectionX, polyline.PolygonPlane.DirectionY);
                polygonCenterProperty.ModifyWithMouse += ModifyLocationWithMouse;

                LengthProperty outerRadius = new LengthProperty(Frame, "Polygon.OuterRadius");
                outerRadius.OnGetValue = () => polyline.PolygonOuterRadius;
                outerRadius.OnSetValue = v => polyline.PolygonOuterRadius = v;
                outerRadius.ModifyWithMouse += ModifyPolygonOuterRadius;
                polygonOuterRadiusHotSpot = new GeoPointHotSpot(polyline.Vertices[0]);
                polygonOuterRadiusHotSpot.StartDragHotspotEvent += OnOuterRadiusHotSpotStartDragHotspot;

                DoubleProperty innerRadius = new DoubleProperty(Frame, "Polygon.InnerRadius");
                innerRadius.OnGetValue = () => polyline.PolygonInnerRadius;
                innerRadius.OnSetValue = v => polyline.PolygonInnerRadius = v;
        ...
        void OnOuterRadiusHotSpotStartDragHotspot(GeoPointHotSpot sender)
        {
            GeneralGeoPointAction gpa = new GeneralGeoPointAction(polyline.Vertices[0], polyline);
            gpa.SetGeoPointEvent += (sender, newValue) =>
            {
                Plane pln = polyline.PolygonPlane;
                GeoPoint cnt = pln.Location;
                double oradius = newValue | cnt;
                double a = (pln.Project(newValue) - pln.Project(cnt)).Angle;
                polyline.SetRegularPolygon(pln, oradius, a, polyline.Vertices.Length);
            };
            Frame.SetAction(gpa);
        }
        void OnInnerRadiusHotSpotStartDragHotspot(GeoPointHotSpot sender)
        {
            GeneralGeoPointAction gpa = new GeneralGeoPointAction(polyline.Vertices[0], polyline);
            gpa.SetGeoPointEvent += (sender, newValue) =>
            {
                ...
                double oradius = newValue | cnt;
                double a = (pln.Project(newValue) - pln.Project(cnt)).Angle;
                int n = polyline.Vertices.Length;
                polyline.SetRegularPolygon(pln, oradius / Math.Cos(Math.PI / n), a - Math.PI / n, n);
            };
            Frame.SetAction(gpa);
        }
        ...
        private void PolylineDidChange(IGeoObject sender, GeoObjectChange change)
        {
            ...
            if (polygonProperty != null && !polyline.IsRegularPolygon)
            {   // die Einträge für das Parallelogramm wegmachen
                polygonProperty = null;
                subEntries = new IPropertyEntry[1];
                subEntries[0] = vertexProperty;
                base.resourceIdInternal = "Polyline.Object";
                base.propertyPage.Refresh(this);
            ...
            if (polyline.IsRegularPolygon && polygonProperty != null)
            {
                polygonOuterRadiusHotSpot.Position = polyline.Vertices[0];
                polygonInnerRadiusHotSpot.Position = new GeoPoint(polyline.Vertices[0], polyline.Vertices[1]);
            }
        }
        ...
        private void ModifyPolygonOuterRadius(IPropertyEntry sender, bool StartModifying)
        {
            GeneralLengthAction gpa = new GeneralLengthAction(sender as LengthProperty, polyline.PolygonPlane.Location, polyline.PolygonPlane.DirectionX, polyline);
            Frame.SetAction(gpa);
        }
        ...
        private void OnPolygonPropertyStateChanged(IPropertyEntry sender, StateChangedArgs args)
        {   // hier erfahren wir, ob die Rechteckeigenschaften aufgeklappt
            // oder zugeklappt werden
            if (args.EventState == StateChangedArgs.State.OpenSubEntries)
            {
                ...
                if (HotspotChangedEvent != null)
                {
                    HotspotChangedEvent(polygonCenterProperty, HotspotChangeMode.Visible);
                    HotspotChangedEvent(polygonOuterRadiusHotSpot, HotspotChangeMode.Visible);
                    HotspotChangedEvent(polygonInnerRadiusHotSpot, HotspotChangeMode.Visible);
                    //HotspotChangedEvent(sizeHotSpot, HotspotChangeMode.Visible);
                }
            }
            else if (args.EventState == StateChangedArgs.State.CollapseSubEntries)
            {
                if (HotspotChangedEvent != null)
                {
                    HotspotChangedEvent(polygonCenterProperty, HotspotChangeMode.Invisible);
                    HotspotChangedEvent(polygonOuterRadiusHotSpot, HotspotChangeMode.Invisible);
                    HotspotChangedEvent(polygonInnerRadiusHotSpot, HotspotChangeMode.Invisible);
                    //HotspotChangedEvent(sizeHotSpot, HotspotChangeMode.Invisible);
                }
            }
        }
