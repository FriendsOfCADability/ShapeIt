...
namespace CADability.Curve2D
{
    ...
    public abstract class GeneralCurve2D : ICurve2D, ISerializable
    {
        ...
        private static void MergeCurveData(IList<GeoPoint2D> points1, IList<GeoVector2D> dirs1, IList<double> params1, IList<GeoPoint2D> points2, IList<GeoVector2D> dirs2, IList<double> params2,
            out List<GeoPoint2D> mergedPoints, out List<GeoVector2D> mergedDirs, out List<double> mergedParams, double tol = 1e-6)
        {
            mergedPoints = new List<GeoPoint2D>();
            mergedDirs = new List<GeoVector2D>();
            mergedParams = new List<double>();

            int i = 0, j = 0;
            while (i < params1.Count && j < params2.Count)
            {
                double u1 = params1[i];
                double u2 = params2[j];
                double diff = u1 - u2;
                ...
                {
                    // Parameter fast gleich → nimm einen (z. B. den ersten)
                    mergedParams.Add(u1);
                    mergedPoints.Add(points1[i]);
                    mergedDirs.Add(dirs1[i]);
                    i++; j++;
                }
                else if (diff < 0)
                {
                ...
                {
                    mergedParams.Add(u2);
                    mergedPoints.Add(points2[j]);
                    mergedDirs.Add(dirs2[j]);
                    j++;
                }
            }

            // Reste anhängen
            for (; i < params1.Count; i++)
            {
                mergedParams.Add(params1[i]);
                mergedPoints.Add(points1[i]);
                mergedDirs.Add(dirs1[i]);
            }

            for (; j < params2.Count; j++)
            {
                mergedParams.Add(params2[j]);
                mergedPoints.Add(points2[j]);
                mergedDirs.Add(dirs2[j]);
            }
        }
        ...
        {   // ACHTUNG: Probleme sind hier Singularitäten und doppelte Punkte. Das muss noch überprüft werden
            ...
            double[] parametersi = GetInflectionPoints();
            if (parametersi != null && parametersi.Length > 0)
            {
                GeoPoint2D[] pointsi = new GeoPoint2D[parametersi.Length];
                GeoVector2D[] directionsi = new GeoVector2D[parametersi.Length];
                for (int i = 0; i < parametersi.Length; ++i)
                {
                    pointsi[i] = PointAt(parametersi[i]);
                    directionsi[i] = DirectionAt(parametersi[i]);
                }
                MergeCurveData(points, directions, parameters, pointsi, directionsi, parametersi,
                    out List<GeoPoint2D> mergedPoints, out List<GeoVector2D> mergedDirections, out List<double> mergedParameters);
            ...
            for (int i = 1; i < parameters.Length; i++)
            {
                ...
                if (Math.Abs(sa0.Radian) > Math.PI / 3 || Math.Abs(sa1.Radian) > Math.PI / 3)
                {
                    double midParam = 0.5 * (parameters[i] + parameters[i - 1]);
                    GeoPoint2D midPoint = PointAt(midParam);
                    GeoVector2D midDir = DirectionAt(midParam);
                    List<GeoPoint2D> newPoints = new List<GeoPoint2D>(points);
                    List<GeoVector2D> newDirs = new List<GeoVector2D>(directions);
                    List<double> newParams = new List<double>(parameters);
                    newPoints.Insert(i, midPoint);
                    newDirs.Insert(i, midDir);
                    newParams.Insert(i, midParam);
                    points = newPoints.ToArray();
                    directions = newDirs.ToArray();
...
//#endif
        ...
        public Polyline2D Debug100Points
        {
            get
            {
                if (interpol == null) MakeTriangulation();
        ...
        public DebuggerContainer Debug100DirPoints
        {
            get
            {
                DebuggerContainer res = new DebuggerContainer();
        ...
        // Second deriavtive approximation
        GeoVector2D rpp(double u, double h)
        {
            if (u < h) u = h; // so that the parameter range is not left
            if (u > 1 - h) u = 1 - h;
            var rpPlus = DirectionAt(u + h);
            var rpMinus = DirectionAt(u - h);
            return new GeoVector2D((rpPlus.x - rpMinus.x) / (2 * h), (rpPlus.y - rpMinus.y) / (2 * h));
        }

        // N(u) = x' y'' - y' x''
        double N(double u, double h)
        {
            var t = DirectionAt(u);
            var tp = rpp(u, h);
            return t.x * tp.y - t.y * tp.x;
        }
        /// <summary>
        ...
        {   // implementation without the use of triangulation, because MakeTriangulation uses GetInflectionPoints
            var list = new List<double>();
            double h = 1e-6; // konstant pro Durchlauf/Intervall
            ...
            for (int i = 1; i < pars.Length; i++)
            {
                ...
                if (nPrev == 0.0 || n == 0.0 || (nPrev < 0 && n > 0) || (nPrev > 0 && n < 0))
                {
                    double a = uPrev, b = u;

                    Func<double, double> g = uu =>
                    {
                        var t = DirectionAt(uu);
                        if (t.Length < 1e-12) return nPrev; // „blockiere“ Singularitäten
                        return N(uu, h);
                    };
            ...
            // old implementation, using MakeTriangulation. We implement MakeTriangulation with the help of GetInflectionPoints, so this would be a circular dependency
            //List<double> res = new List<double>();
            //if (interpol == null) MakeTriangulation();
