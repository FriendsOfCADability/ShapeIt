...
namespace CADability
{
    ...
    public class ModelView : IShowPropertyImpl, ICommandHandler, IView, IActionInputView
    {
        ...
        private bool tempFixPointValid = false; // will be true when rotating the view and a fixpoint in the middle of the screen is used
        ...
        void IView.OnMouseDown(MouseEventArgs eIn)
        {
            ...
            tempFixPointValid = false; // will be set true when rotating the view and a temporary fixpoint has been found
            ...
            if (doDirection) // && paintToOpenGl != null)
            {
                ...
                if (VOffset != 0 || HOffset != 0)
                {
                    ...
                    if (distance == 0.0)
                    {
                        ...
                        if (fixPointValid || tempFixPointValid)
                        ...
                        else
                        {
                            ...
                            if (pd != double.MaxValue)
                            {
                                fixPoint = fp = pa.FrontCenter + pd * pa.Direction; // the point in the center of the screen, when there is something
                                tempFixPointValid = true;
                            }
                            else
