...
namespace CADability.GeoObject
{
        ...
        IExportStep, ISurfaceOfArcExtrusion, ICylinder, IJsonSerialize
    {
        ...
        internal CylindricalSurface(ModOp toCylinder, BoundingRect? domain = null) : base(domain)
        {
        ...
        {
            return new CylindricalSurface(m * toCylinder, domain);
        }
        /// <summary>
                        ...
                        {   // es gibt 4 halb-Ellipsen (oder 2 ganze Ellipsen, die sich aber schneiden, und das ist hier nicht erwünscht)
                            ...
                            for (int i = 0; i < 4; i++)
                            {
                                ...
                                {   // these two points are the points where the two ellipses intersect. We return 4 ellipse arcs
                                    // BRepIntersection needs this two points as additional vertices, add them if they dont exist
                                    if (seeds == null) seeds = new List<GeoPoint>();
                                    bool foundsp = false, foundep = false;
                                    for (int j = 0; j < seeds.Count; j++)
                                    {
                                        foundsp |= Precision.IsEqual(seeds[j], elli.StartPoint);
                                        foundep |= Precision.IsEqual(seeds[j], elli.EndPoint);
                                    }
                                    if (!foundsp) seeds.Add(elli.StartPoint);
                                    if (!foundep) seeds.Add(elli.EndPoint);
                                }
                                // 
                                ICurve2D sc1 = this.GetProjectedCurve(elli, 0.0);
                                SurfaceHelper.AdjustPeriodic(this, thisBounds, sc1);
                                // ProjectedCurve pc1 = new CADability.ProjectedCurve(elli, this, true, thisBounds);
                                BoundingRect common = BoundingRect.Common(sc1.GetExtent(), thisBounds);
                                if (common.Width * common.Height > Precision.eps)
                                {
                                    // ProjectedCurve pc2 = new CADability.ProjectedCurve(elli, cyl2, true, otherBounds);
                                    ICurve2D sc2 = cyl2.GetProjectedCurve(elli, 0.0);
                                    SurfaceHelper.AdjustPeriodic(other, otherBounds, sc2);
                                    common = BoundingRect.Common(sc2.GetExtent(), otherBounds);
                                    if (common.Width * common.Height > Precision.eps)
                                    {
                                        lres.Add(new DualSurfaceCurve(elli, this, sc1, cyl2, sc2));
                                    }
                                }
                            }
                            return lres.ToArray();
        ...
        {
            return new CylindricalSurface(toCylinder, domain);
        }
        /// <summary>
        ...
        public override ICurve2D GetProjectedCurve(ICurve curve, double precision)
        {
            ...
            else if (curve is Ellipse)
            {
                ...
                {
                    // an ellipse on a cylinder is a sine curve in 2d, if the ellipse is on the cylinder and not somewhere else
                    if (Math.Abs(e.Center.x) + Math.Abs(e.Center.y) < Precision.eps && Math.Abs(e.MinorRadius - 1) < Precision.eps)
                    {   // this ellipse in on the cylinder, so we can define a sine curve
                        GeoPoint2D[] testPoints = new GeoPoint2D[4]; // we use 4 testpoints to get a continuous  parameter interval
                        BoundingRect ext = BoundingRect.EmptyBoundingRect;
                        for (int i = 0; i < 4; i++)
                        {
                            testPoints[i] = PositionOfUnit(e.PointAt(i / 3.0));
                        ...
                        double tx2 = tx1 + Math.PI; // we could decide better, but this is easier
                        SineCurve2D sc1 = new SineCurve2D(testPoints[0].x, testPoints[3].x - testPoints[0].x, new ModOp2D(1, 0, tx1, 0, fy, e.Center.z));
                        ...
                        if (sc != null)
                        {
                            ...
                            if (!domain.IsEmpty()) SurfaceHelper.AdjustPeriodic(this, domain, sc); // must be adjusted to domain
                            return sc;
                        }
                    }
                }
