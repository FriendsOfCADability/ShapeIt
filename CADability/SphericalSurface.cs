...
namespace CADability.GeoObject
{
    ...
    public class SphericalSurface : ISurfaceImpl, ISerializable, IDeserializationCallback, IExportStep, ISphere
    {
        ...
        {   // hier sollten die Schnitte mit Ebene, Cylinder, Kegel und Kugel gelöst werden
            ...
            {
                // intersect two spheres: this is a circle (or empty)
                List<IDualSurfaceCurve> res = new List<IDualSurfaceCurve>();
                GeoVector v = sp.Location - Location;
                double r1 = RadiusX, r2 = sp.RadiusX;

                double d = v.Length;

                // ignore special cases: same centers, too far apart, one inside the other
                if (d > Precision.eps && (d < r1 + r2 + Precision.eps) && (d > Math.Abs(r1 - r2) - Precision.eps))
                {

                    // Einmaliges/zweifaches Schneiden:
                    // Abstand a vom Mittelpunkt c1 entlang der Achse zu c2 bis zur Schnitt-Ebene
                    // a = (r1^2 - r2^2 + d^2) / (2d)
                    double a = (r1 * r1 - r2 * r2 + v.LengthSqared) / (2.0 * d);

                    // Radius^2 des Schnittkreises: h^2 = r1^2 - a^2
                    double h2 = r1 * r1 - a * a;

                    if (h2 > 0)
                    {
                        double radius = Math.Sqrt(h2);

                        // Einheitsrichtung von c1 nach c2
                        ...
                        // when more seeds are provided, I don't know what to do with them
                        else if (seeds.Count == 0)
                        {   // here we could clip to the bounds of both surfaces
                        }
                        return res.ToArray();
                    }
                }
